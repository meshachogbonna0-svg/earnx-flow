...
function AdminPage() {
  ...
  const load = async () => {
    ...
    const { data: adminOk, error: adminError } = await supabase.rpc("ensure_admin_role");
    console.log("[Admin] Logged-in user:", auth.user.email, auth.user.id);
    console.log("[Admin] RPC result:", adminOk, adminError);
    if (adminError) {
      throw new Error(`Admin authorization error: ${adminError.message}`);
    }
  ...
  return (
    ...
    >
      {/* Rebuilt Admin Command Center */}
      <div className="mb-4 overflow-hidden rounded-3xl border border-gold/25 bg-gradient-to-br from-navy via-card to-navy-deep shadow-xl">
        <div className="flex items-center justify-between gap-3 p-4 sm:p-5">
          <div className="min-w-0">
            <div className="mb-1 flex items-center gap-2">
              ...
              <span className="text-[10px] font-bold uppercase tracking-[0.18em] text-gold">EarnX Control Center</span>
            </div>
            <h1 className="font-display text-lg font-extrabold tracking-tight">Admin Command Center</h1>
            <p className="mt-1 text-[10px] leading-relaxed text-muted-foreground">
              Manage users, payouts, rewards, content, promotions and platform settings from one place.
            </p>
          </div>
          <button
            type="button"
            onClick={() => setMenuOpen(true)}
            className="grid h-11 w-11 shrink-0 place-items-center rounded-2xl border border-gold/30 bg-gold/10 text-gold shadow-sm transition active:scale-95"
        ...
        <div className="grid grid-cols-2 gap-2 border-t border-border/50 p-3 sm:grid-cols-4">
          {[
            ["Users", num("total_users") || users.length, "users"],
            ["Pending", (num("pending_activations") + num("pending_upgrades") + num("pending_withdrawals")), "withdrawals"],
            ["Paid out", naira(num("total_withdrawn")), "transactions"],
            ["Wallet", naira(num("total_balance") || users.reduce((s, u) => s + Number(u.balance), 0)), "users"],
          ].map(([label, value, target]) => (
            <button
              key={String(label)}
              type="button"
              onClick={() => setSection(target as Section)}
              className="rounded-2xl border border-border/60 bg-background/20 p-3 text-left transition hover:border-gold/30 active:scale-[0.98]"
            >
              <p className="text-[9px] font-semibold uppercase tracking-wider text-muted-foreground">{label}</p>
              <p className="mt-1 truncate text-sm font-extrabold">{String(value)}</p>
            </button>
          ))}
        </div>
      </div>
      ...
      {/* Navigation drawer */}
      <div className={cn("fixed inset-0 z-[70]", !menuOpen && "pointer-events-none")} aria-hidden={!menuOpen}>
        <button
          type="button"
        ...
        <aside
          className={cn(
            "absolute inset-y-0 right-0 flex w-[88%] max-w-sm flex-col border-l border-gold/20 bg-gradient-to-b from-navy via-card to-navy-deep shadow-2xl transition-transform duration-300",
            menuOpen ? "translate-x-0" : "translate-x-full",
          )}
        >
          <div className="flex items-center justify-between border-b border-border/60 px-5 py-4">
            <div>
              <p className="font-display text-sm font-extrabold">Admin navigation</p>
              <p className="mt-0.5 text-[10px] text-muted-foreground">Choose a control area</p>
            </div>
            <button
              type="button"
              aria-label="Close"
          ...
          <nav className="flex-1 overflow-y-auto p-3">
            <div className="mb-3 rounded-2xl border border-gold/20 bg-gold/5 p-3">
              <p className="text-[9px] font-bold uppercase tracking-[0.16em] text-gold">Operations</p>
              <div className="mt-2 grid grid-cols-2 gap-1.5">
                {menu.slice(0, 8).map(({ key, label, icon: Icon }) => (
                  <button
                    key={key}
                    type="button"
                    onClick={() => { setSection(key); setMenuOpen(false); }}
                    className={cn(
                      "flex items-center gap-2 rounded-xl px-2.5 py-2.5 text-left text-[10px] font-semibold transition",
                      section === key ? "bg-gold-gradient text-gold-foreground" : "bg-background/30 text-muted-foreground hover:bg-secondary",
                    )}
                  >
            ...
            <p className="px-2 pb-2 pt-1 text-[9px] font-bold uppercase tracking-[0.16em] text-muted-foreground">Platform</p>
            <div className="space-y-1">
              {menu.slice(8).map(({ key, label, icon: Icon }) => (
                <button
                  key={key}
                  type="button"
                  onClick={() => { setSection(key); setMenuOpen(false); }}
          ...
          <div className="border-t border-border/60 p-3">
            <button
              type="button"
              onClick={() => void load()}
              className="flex w-full items-center justify-center gap-2 rounded-xl border border-gold/30 bg-gold/10 py-3 text-xs font-bold text-gold"
            >
              <TrendingUp className="h-3.5 w-3.5" /> Refresh control center
            </button>
          </div>
        </aside>
      </div>
      ...
      <div className="mb-3 flex items-center justify-between rounded-2xl border border-border/60 bg-card/70 px-3.5 py-3">
        <div>
          <p className="text-[9px] font-bold uppercase tracking-[0.16em] text-gold">Current area</p>
          <p className="mt-0.5 text-xs font-bold">{menu.find((m) => m.key === section)?.label}</p>
        </div>
        <button
          type="button"
          onClick={() => setMenuOpen(true)}
          className="rounded-xl border border-border px-3 py-2 text-[10px] font-bold text-muted-foreground"
        >
          Change
        </button>
      </div>
        ...
        <section className="space-y-3 rounded-2xl border border-border bg-card p-4">
          ...
          {(
            [
              ...
              ["activated_starter_pack_reward", "Activated starter pack reward (₦)"],
          ...
          <div className="space-y-2 rounded-xl border border-gold/20 bg-gold/5 p-3">
            ...
            {([
              ...
              ["activation_success_message", "Activation success message"],
              ["activation_rejection_message", "Activation rejection message"],
              ["upgrade_processing_message", "Upgrade processing message"],
              ["upgrade_success_message", "Upgrade success message"],
              ["upgrade_rejection_message", "Upgrade rejection message"],
              ["withdrawal_processing_message", "Withdrawal processing message"],
              ["withdrawal_success_message", "Withdrawal success message"],
              ["withdrawal_rejection_message", "Withdrawal rejection message"],
