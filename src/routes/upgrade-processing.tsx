...
function UpgradeProcessingPage() {
  ...
  useEffect(() => { let alive=true; const load=async()=>{ const {data:auth}=await supabase.auth.getUser(); if(!auth.user) return navigate({to:"/login",replace:true}); const {data}=await supabase.from("upgrade_requests").select("status, admin_note, requested_level, amount").eq("user_id",auth.user.id).order("created_at",{ascending:false}).limit(1).maybeSingle(); if(!alive)return; const r=data as Req|null; setReq(r); setLoading(false); if(r?.status==="approved" || (r && r.requested_level <= Number(level))) navigate({to:"/upgrade-confirmation",search:{level:String(r.requested_level)},replace:true}); }; void load(); const t=window.setInterval(()=>void load(),5000); return()=>{alive=false;window.clearInterval(t)}; },[navigate,level]);
