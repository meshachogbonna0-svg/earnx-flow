...
function RequestsPage() {
  ...
  const newestPending = items.find((i) => stateOf(i.status) === "pending");

  return (
    <AppPage title="Request Status" subtitle="Activation, upgrades and payouts">
      {newestPending && (
        <section className="animate-fade-up rounded-3xl border border-gold/35 bg-gradient-to-br from-navy via-card to-navy-deep p-5 text-center shadow-gold-glow">
          <span className="mx-auto grid h-12 w-12 place-items-center rounded-full bg-gold/10 text-gold"><Hourglass className="h-6 w-6 animate-pulse" /></span>
          <p className="mt-3 text-[10px] font-bold tracking-[0.2em] text-gold">REQUEST RECEIVED</p>
          <h2 className="mt-2 text-base font-extrabold">{newestPending.kind === "activation" ? "Activation" : newestPending.kind === "upgrade" ? "Upgrade" : "Withdrawal"} processing</h2>
          <p className="mx-auto mt-2 max-w-sm text-[11px] leading-relaxed text-muted-foreground">Our team has received your request and it is currently being processed. You will see the result here when our team completes the review.</p>
        </section>
      )}
